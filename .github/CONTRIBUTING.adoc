...
. Fork the {url-imdg-docs}[`hazelcast-reference-manual` repository]
...
+
NOTE: This script builds the documentation only for your local branch. If you want to build the whole site, you can run the script from the `docs-poc` repository.
...
=======
<1> The `antora.yml` file tells Antora that the contents of the `modules/` folder should be processed and added to the documentation site. This file is called the https://docs.antora.org/antora/2.3/component-version-descriptor[component version descriptor file].
<2> The https://docs.antora.org/antora/2.3/module-directories[`modules/` folder] contains the content that Antora will process
<3> The https://docs.antora.org/antora/2.3/root-module-directory[`ROOT/` folder] contains any content that can't be categorized uner a specfic module name. Unlike other modules, the name of this module is never displayed in the URL of the site.
<4> The https://docs.antora.org/antora/2.3/asciidoc/link-attachment[`attachments/` folder] contains downloadable content such as ZIP files that a user can download through a link. To include a download link, use the following syntax:
+
[source,asciidoc]
----
link:path/to/example.zip[Download the example]
----

<5> The https://docs.antora.org/antora/2.3/page/examples-and-source-snippets[`examples/` folder] contains examples such as source code that you can include in Asciidoc pages, using the following syntax:
+
[source,asciidoc]
----
include::example$example-filename.json[]
----

<6> The https://docs.antora.org/antora/2.3/page/images[`images/` folder] contains images that you can include in Asciidoc pages, using the following syntax:
+
[source,asciidoc]
.Block images
----
...
image:path/to/image[Alt text]
----
<7> The https://docs.antora.org/antora/2.3/page[`pages/` folder] contains the Asciidoc pages that make up the documentation project.
<8> The `partials/` folder contains Asciidoc snippets such as warnings or copyright material that you can include in any Asciidoc pages, using the following syntax: `include::partial$partial-filename.adoc[]`
<9> The https://docs.antora.org/antora/2.3/navigation[`nav.adoc`] file contains a list of links that will become part of the left-hand navigation menu.
