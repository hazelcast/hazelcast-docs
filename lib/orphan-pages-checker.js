...
const [DOCS_FOLDER = 'docs'] = process.argv.slice(-1);
...
class NavLinksCollector {
	...
	static buildChildNavUrl( navModule) {
		...
		return path.join('docs', 'modules', navModule, 'partials', 'nav.adoc');
	}
...
function main() {
	...
	const linksCollector = new NavLinksCollector(path.join('docs', 'modules', 'ROOT', 'nav.adoc'));
